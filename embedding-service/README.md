...
Python FastAPI wrapper around OpenAI's CLIP. Same model produces both image and text embeddings, which is what makes hybrid search work.
...
| Method | Path | Purpose |
|---|---|---|
| `POST` | `/embed` | Image → 512-dim vector (fast path, used during ingestion if you don't need tags) |
| `POST` | `/embed-and-tag` | Image → vector + auto-caption + auto-tags (used by both index and query) |
| `POST` | `/embed-text` | Text → 512-dim vector in the **same space** as image embeddings |
...
1. CLIP encodes the image into a 512-dim vector.
...
First start downloads ~600 MB of CLIP weights to `~/.cache/huggingface/`. Cached for subsequent starts.
...
- CPU inference is ~50–200 ms per image on Apple Silicon, 200–400 ms on x86. Acceptable for the demo.
