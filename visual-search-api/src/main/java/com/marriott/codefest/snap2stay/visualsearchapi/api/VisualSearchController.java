...
public class VisualSearchController {
    ...
    public Mono<VisualSearchDtos.VisualSearchResponse> searchMultipart(
            ...
            @RequestPart(value = "marketCode", required = false) String marketCode) throws IOException {
        ...
        log.info("visualSearch (multipart) bytes={} useImageLocation={} destination={}",
                bytes.length, optInLocation, destination);

        return pipeline.search(bytes, optInLocation, filters, dates)
